		...
		protected void Start()
		{
			...
			if (physicalRigidbodyPrefab != null)
			{
				...
				while (rigidbodies.Count < physicalRigidbodyCount && tries < physicalRigidbodyCount * 10)
				{
					...
					var s = Random.Range(minPhysicalRigidbodyScale, maxPhysicalRigidbodyScale);
				...
				for (int i = 0; i < rigidbodies.Count; ++i)
				{
					...
					var speed = Random.Range(minPhysicalRigidbodySpeed, maxPhysicalRigidbodySpeed);
					rigidBody.velocity = new Vector3(Mathf.Cos(direction) * speed, 0f, Mathf.Sin(direction) * speed);
					rigidBody.angularVelocity = new Vector3(
						Random.Range(minPhysicalRigidbodyRotation, maxPhysicalRigidbodyRotation),
						Random.Range(minPhysicalRigidbodyRotation, maxPhysicalRigidbodyRotation),
						Random.Range(minPhysicalRigidbodyRotation, maxPhysicalRigidbodyRotation));
			...
			if (virtualRigidbodyPrefab != null)
			{
				for (int i = 0; i < virtualRigidbodyCount; ++i)
				{
					...
					var y = Random.Range(0, 1000000) / 1000000f;
					var p = world.transform.TransformPoint(world.axis0 * x + world.axis1 * y + world.origin);
					var s = Random.Range(minVirtualRigidbodyScale, maxVirtualRigidbodyScale);
					...
					var rigidBody = rigidbody.GetComponent<Rigidbody>();
					var direction = Random.Range(0, 1000000) / 1000000f * Mathf.PI * 2f;
					var speed = Random.Range(minVirtualRigidbodySpeed, maxVirtualRigidbodySpeed);
					rigidBody.velocity = new Vector3(Mathf.Cos(direction) * speed, 0f, Mathf.Sin(direction) * speed);
					rigidBody.angularVelocity = new Vector3(
						Random.Range(minVirtualRigidbodyRotation, maxVirtualRigidbodyRotation),
						Random.Range(minVirtualRigidbodyRotation, maxVirtualRigidbodyRotation),
						Random.Range(minVirtualRigidbodyRotation, maxVirtualRigidbodyRotation));
